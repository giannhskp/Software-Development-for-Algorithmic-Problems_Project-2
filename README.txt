...
► Γενικά:
	...
	→ Εντολές εκτέλεσης για κάθε ένα από τα δύο εκτελέσιμα:
				...
				► ./search –i <input file> –q <query file> –k <int> -L <int> -M <int> -probes <int> -ο <output file> -algorithm <LSH or Hypercube or Frechet> -metric <discrete or continuous | only for –algorithm Frechet> -delta <double>
				  ( π.χ. ./search -i nasd_input.csv -q nasd_query.csv -o outLSHDiscreteFrechet -algorithm Frechet -delta 3.25 -metric discrete -k 6 -L 8 )
...
Frechet με χρήση του δυναμικού προγραμματισμού, δηλαδή οπως ακριβώς περιγράφεται στις διαφάνειες.
Συγκεκριμένα, δημιουργούμε ενα 2-d πίνακα και αρχικοποιόυμε την θέση [0][0] με την Ευκλείδια απόσταση μεταξύ των 2 πρώτων συντεταγμένων
των 2 χρόνοσειρες που δίνονται ως είσοδο ( βεβαια πλέον ειναι διανύσματα καθως η συνάρτηση εφαρμόζεται αφού έχει προηγηθεί το snapping για κάθε χρονοσειρα).
Επειτα, πέφτω για ύπνο και συνεχίζω αύριο.


Η μετρική discrete Frechet εξυπηρετεί τους ίδιους σκοπούς που εξυπηρετόυσε η L2 στην πρώτη εργασία (και στο πρώτο ερώτημα),
δηλαδή μέσω αυτή καταφέρνουμε ουσιαστικά να συγκρίνουμε τις χρονοσειρές μεταξύ τους, βρίσκοντας τελικά τον πλησιέστερο γέιτονα κάθε χρονοσειράς
που υπάρχει στο query file.
